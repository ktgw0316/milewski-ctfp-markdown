...
カン拡張には別の解釈もある。関手$K$が圏$\cat{I}$を$\cat{A}$の中に埋め込むとする。最も単純なケースでは、$\cat{I}$は単に$\cat{A}$の部分圏になる。$\cat{I}$を$\cat{C}$に写す関手$D$がある。$D$を$\cat{A}$全体で定義された関手$F$に拡張できるだろうか？
理想的には、そのような拡張によって合成$F \circ K$が$D$と同型になる。言い換えると、$F$は$D$の始域を$\cat{A}$に拡張する。しかし、本格的な同型は通常求めすぎで、半分で十分だ。つまり、$F \circ K$から$D$への片道の自然変換$\varepsilon$だけでよい。（逆方向は左カン拡張が指す。）
...
カン拡張の興味深い応用として自由関手 (free functor) の構成がある。これは以下のような実際的な問題の解となる：対象の写像である型コンストラクターがあるとする。その型コンストラクターに基づいて関手を定義可能だろうか？
