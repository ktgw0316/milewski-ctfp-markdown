...
`\end{figure}`{=latex}
\noindent
当然、この埋め込みの描像は、極限のような例では破綻する。関手$K$が対象の単射ではなく、すなわちホム集合の忠実関手ではないからだ。その場合、カン拡張は失われた情報を推定するために最善を尽くす。
...
カン拡張を使えば関手の作用をもとの始域の外側に拡張できる、という考え方を再検討してみよう。$K$が$\cat{I}$を$\cat{A}$の中に埋め込むとする。関手$D$は$\cat{I}$を$\Set$に写す。$K$の像内の任意の対象$a$について$a = K i$であり、拡張された関手は$a$を$D i$に写すと言える。問題は、$K$の像の外側にある$\cat{A}$内の対象をどうするかだ。そのような対象はどれもたくさんの射を通じて$K$の像内のすべての対象と潜在的に繋がっている、と考えればよい。関手はそれらの射を保存しなければならない。対象$a$から$K$の像への射の全体は次のホム関手によって特徴づけられる：
$$\cat{A}(a, K -)$$
...
鍵となる観察は、型構成子は離散圏を始域とする関手として表現できる、というものだ。離散圏の射は恒等射だけだ。任意の圏$\cat{C}$について、単に恒等射以外の射を捨て去れば、常に離散圏$\cat{|C|}$を構成できる。したがって、$\cat{|C|}$から$\cat{C}$への関手$F$は、対象の単純な写像、すなわちHaskellで型構成子と呼ばれるものになる。また、標準関手 (canonical functor) $J$も存在し、$\cat{|C|}$を$\cat{C}$内へ注入する。それは対象についての（および恒等射についての）恒等関手だ。$F$の$J$に沿った左カン拡張は、存在するなら、$\cat{C}$から$\cat{C}$への関手だ：
