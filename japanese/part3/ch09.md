...
\noindent
簡単な例を見てみよう。リスト自己関手の代数は、ある型`a`と、`a`のリストから`a`を生成する関数とで構成される。この関数が`foldr`を使って表せるとしよう。そのためには、要素の型とアキュムレーターの型を、両方とも同じ`a`になるように選ぶ。
...
覚えているかもしれないが、すべての随伴はモナドを定義する。$F^T$と$U^T$の間の随伴は、アイレンバーグ・ムーア圏の構成で使われたまさにそのモナド$T$を定義することが知られている。この構成はすべてのモナドに対して行えるので、すべてのモナドは随伴から生成できる、といえる。後ほど、同じモナドを生成する別の随伴が存在することを示す。
...
\end{gathered}
$$
があるとき、これらは圏$\cat{C}$における以下の2本の射に対応している。
$$
\begin{gathered}
f \Colon a \to T b \\
g \Colon b \to T c
...
\noindent
このレンズ則は、データ構造$a$のフィールドをもともとの値に設定しても何も変わらないという事実を表す。この規則はGETPUTとしても知られる。
...
\noindent
は、フィールドの値を2回設定するのは1回設定するのと同じだということを表す。この規則はPUTPUTとしても知られる。2つ目
...
\noindent
は、$s$に設定されたフィールドの値を取得すると$s$が返されることを表す。この規則はPUTGETとしても知られる。

GETPUTとPUTGETを満たすレンズは、振る舞いの良い (well-behaved) レンズと呼ばれる。そのレンズがさらにPUTPUTも満たすなら、とても振る舞いの良い (very well behaved) レンズと呼ばれる[^very-well-behaved]。つまり、言い換えれば、とても振る舞いの良いレンズはまさに`store`関手のコモナド余代数である。

[^very-well-behaved]:
  訳注：参考：J. Nathan Foster, Michael B. Greenwald, Jonathan T. Moore, Benjamin C. Pierce, and Alan Schmitt. 2007. *Combinators for bidirectional tree transformations: A linguistic approach to the view-update problem*. ACM Trans. Program. Lang. Syst. 29, 3 (May 2007), 17–es. <https://doi.org/10.1145/1232420.1232424>

## 課題

1. 自由関手$F^T \Colon C \to C^T$の射に対する作用は何か。ヒント：モナドの$\mu$について自然性条件を使う。
1. 次の随伴を定義せよ。
   $$U^W \dashv F^W$$
1. 上記の随伴がもとのコモナドを再現することを証明せよ。
