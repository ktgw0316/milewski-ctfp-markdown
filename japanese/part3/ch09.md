...
\noindent
$\cat{C}$から$\cat{C}_T$への関手$F$があり、対象に対しては自明な働きをする。射に対しては、$f$の戻り値を装飾するKleisli矢を作成することによって、$\cat{C}$内の$f$を$\cat{C}_T$内の射に写す。次のような射が与えられたとき：
$$f \Colon a \to b$$
対応するKleisli矢を使って$\cat{C}_T$内の射ができる：
$$\eta \circ f$$
Haskellでは次のように記述できる：
...
\noindent
また、$\cat{C}_T$から$\cat{C}$へ戻る関手$G$も定義できる。それはKleisli圏から対象$a$を取り、$\cat{C}$内の対象$T a$に写す。Kleisli矢：
$$f \Colon a \to T b$$
...
同様に、余Kleisli矢から余Kleisli圏を構築でき、対応する随伴からコモナドを再生成できる。
