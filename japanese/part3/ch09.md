...
\noindent
は、$s$に設定されたフィールドの値を取得すると$s$が返されることを表す。
この規則はPUTGETとしても知られる。

GETPUTとPUTGETを満たすレンズは、振る舞いの良い (well-defined) レンズと呼ばれる。
そのレンズがさらにPUTPUTも満たすなら、とても振る舞いの良い (very well behaved) レンズと呼ばれる[^very-well-behaved]。
つまり、言い換えれば、とても振る舞いの良いレンズはまさに`store`関手のコモナド余代数である。
