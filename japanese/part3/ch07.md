...
Kleisli矢は値を受け取り、文脈で装飾した結果を生成する。同様に、余Kleisli矢は文脈全体と共に値を受け取り、結果を生成する。これは[*文脈付き計算*]{.keyword #contextual_computation}\index{文脈付き計算} (contextual computation) の実施例だ。
