...
`Store`の`Reader`の部分は、一般化されたコンテナに型`s`の要素をキーにして`a`が格納されていると見なせる。たとえば、`s`が`Int`なら、`Reader Int a`は`a`の無限双方向ストリームだ。`Store`はこのコンテナとキー型の値とをペアにする。また、たとえば、`Reader Int a`はある`Int`とペアになっている。この場合、`extract`はその整数を使って無限ストリームにインデックスを作成する。`Store`の2番目の要素は現在位置と見なせる。
