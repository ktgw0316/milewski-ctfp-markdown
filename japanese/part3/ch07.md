...
\noindent
次は`extend`を解剖できるだろうか？　なぜ単に関数`w a -> b`を引数`w a`に適用しないのか、と言いたくなるかもしれない。しかし、結果として得られる`b`を`w b`に変換する方法がないことにすぐ気付くだろう。コモナドには値を持ち上げる手段がないことを思い出してほしい。ここではモナドに類似した構成で`fmap`を使った。ここで`fmap`が使えるとすれば、型`w (w a)`の何かが自由に使える場合だけだ。`w a`を`w (w a)`に変えられさえすればよい。そして、便利なことに、それはまさに`join`の双対だ。それは`duplicate`と呼ばれる：
