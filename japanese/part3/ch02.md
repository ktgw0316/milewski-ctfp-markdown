...
より正確には、2つの対象$a$と$b$の積は2つの射$\mathit{fst}$と$\mathit{snd}$を伴う対象 $(a \times b)$（すなわちHaskell表記の`(a, b)`）で、かつ2つの射$p \Colon c \to a$と$q \Colon c \to b$を伴う他の候補$c$に対して、$p$と$q$を$\mathit{fst}$と$\mathit{snd}$を通じて分解する一意な射$m \Colon c \to (a, b)$ が存在するようなものである。
