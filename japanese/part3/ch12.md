...
まず、合成の定義から始めよう。通常は、$\cat{C}(b, c)$ からの射1本と$\cat{C}(a, b)$ からの射1本からなる射のペアを取り、$\cat{C}(a, c)$ からの射に写す。言い換えれば、次の写像だ。
...
このテンソル積は結合的なのが望ましいが、自然同型の違いを除いた結合律さえ満たせば十分だ。この同型射は結合切替子と呼ばれる。その成分
...
\end{aligned}
$$
結合切替子と単位積子は以下のコヒーレンス条件を満たす必要がある。
...
\noindent
<!-- -->
[*対称*]{.keyword #symmetric}\index{対称} (symmetric) モノイダル圏と呼ばれるための条件は、以下の成分
...
恒等射についてはどうだろう？　それは$1$から$\cat{C}(a, a)$ への射だ。$1$からの射は1本だけで、恒等射$\idarrow[1]$なので、$\cat{C}(a, a)$ は$1$である必要がある。これは$a \leqslant a$であることを意味する。すなわち、前順序の反射律だ。したがって、前順序を豊饒圏として実装すると、推移律と反射律の両方が自動的に強制される。
...
では合成について語ろう。隣接する2つのhom対象のテンソル積$\cat{C}(b, c) \otimes \cat{C}(a, b)$ から始める。テンソル積は、2つの距離の合計として定義された。合成はこの積から$\cat{C}(a, c)$ への$\cat{V}$内の射だ。$\cat{V}$内の射は「大なりイコール」関係として定義されている。言い換えると、$a$から$b$への距離と$b$から$c$への距離の合計は、$a$から$c$への距離以上になる。もっとも、これは標準的な三角不等式にすぎない。良し！
