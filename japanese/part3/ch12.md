...
それらの数値が対象であるようなモノイダル圏$\cat{V}$さえ構築できればよい。非負実数（と無限大）は全順序を形成するので、細い圏として扱える。このような2つの数$x$と$y$の間の射は、$x \geqslant y$の場合だけ存在する（註：これは前順序の定義での伝統的な向きとは逆だ）。モノイダル構造は加算で与えられ、0が単位対象として機能する。言い換えると、2つの数値のテンソル積はそれらの合計だ。
