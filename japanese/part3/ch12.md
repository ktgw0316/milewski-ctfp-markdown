...
このテンソル積は結合的なのが望ましいが、自然同型を除いた結合律さえ満たせば十分だ。この同型射は結合切替子と呼ばれる。その成分はこうなる。
...
ここで、内部hom${[}a, c{]}$の定義で出てきた随伴を使った。この新しい集合に射を構築できるなら、随伴はもとの集合の射を指し、そして、その射は合成として使える。その射は、自由に使えるいくつかの射を合成することによって構築される。まず、結合切替子$\alpha_{[b, c]\ [a, b]\ a}$を使って左側の式を再結合できる。
