...
もちろん、これは話の半分にすぎない。なぜなら、関手は対象だけでなく射も写すからだ。では自然変換はこれらの写像をどうするのだろうか？　射の写像は固定されていて、$F$と$G$の間の自然変換では$F f$が$G f$に変換されなければならないことが分かる。さらに、2つの関手による射の写像は、それに適合する自然変換を定義する際の選択肢を大幅に制限する。$\cat{C}$の2つの対象$a$と$b$の間に射$f$があるとする。それは$\cat{D}$内の2つの射$F f$と$G f$に写される：
$$
\begin{gathered}
F f \Colon F a \to F b \\
G f \Colon G a \to G b
...
\noindent
共変にしろ反変にしろ、関手ではない型コンストラクターは存在するのだろうか？　次のような例が挙げられる：
...
圏論のルールを一言で言うと、合成に出会うたびに圏を探すべきだ、となる。自然変換には垂直合成があり、それは関手圏の一部だ。しかし、水平合成はどうだろうか？　それはどの圏にあるのだろう？
