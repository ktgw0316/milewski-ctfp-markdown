...
プログラミングにおける関手（より正確には自己関手）の役割についてはすでに述べた。それらは型を型に写すような型構成子に対応する。また、そういった関手は関数を関数に写すが、この写像は高階関数`fmap`（あるいはC++における`transform`や`then`^[訳注：`std::experimental::future<T>::then`を指す。]に類するもの）によって実装される。
