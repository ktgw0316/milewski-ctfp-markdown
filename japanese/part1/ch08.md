...
\noindent
これは2つの型の和だ。和が関手的なのは知っている。1つ目の部分である`Nothing`は、`Const ()`の`a`に対する作用として表せる（`Const`の1番目の型パラメーターはunit型に設定されている――後で`Const`のさらに興味深い使い方を説明する）。2つ目の部分は、恒等関手の別名だ。`Maybe`は、同型を除いて、次のようにも定義できる。
