...
しかし、ありきたりの抽象化ではだめだ――[関数合成]{.keyword #function_composition}自体を抽象化する必要がある。しかし、合成は圏論の本質なので、さらにコードを書く前に、圏の観点から問題を分析してみよう。
...
`compose`テンプレートへの型の受け渡しにはまだ多くのノイズがある。これは、戻り値型推論を持つ総称ラムダ関数をサポートするC++14準拠のコンパイラがあれば回避できる。(このコードはEric Nieblerによる。)
