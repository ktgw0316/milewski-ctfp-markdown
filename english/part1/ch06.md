...
These observations can be formalized by saying that $\Set$ (the category of sets) is a [monoidal category]{id="monoidal_category"}. It's a category that's also a monoid, in the sense that you can multiply objects (here, take their Cartesian product). I'll talk more about monoidal categories, and give the full definition in the future.
...
Programming with tuples and multi-argument constructors can get messy and error prone — keeping track of which component represents what. It's often preferable to give names to components. A product type with named fields is called a [record]{id="record"} in Haskell, and a `struct` in C.
...
Mathematicians have a name for two such intertwined monoids: it's called a [semiring]{id="semiring"}. It's not a full [ring]{id="ring"}, because we can't define subtraction of types. That's why a semiring is sometimes called a [rig]{id="rig"}, which is a pun on “ring without an _n_” (negative). But barring that, we can get a lot of mileage from translating statements about, say, natural numbers, which form a rig, to statements about types. Here's a translation table with some entries of interest:
