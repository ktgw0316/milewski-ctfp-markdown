...
Now we have two ways of getting from $F\ a$ to $G\ b$. To make sure that they are equal, we must impose the [naturality condition]{.keyword #naturality_condition} that holds for any $f$:
...
C++, on the other hand, supports by default [ad hoc polymorphism]{.keyword #ad_hoc_polymorphism}, which means that a template doesn't have to be well-defined for all types. Whether a template will work for a given type is decided at instantiation time, where a concrete type is substituted for the type parameter. Type checking is deferred, which unfortunately often leads to incomprehensible error messages.
...
However, function types are not covariant in the argument type. They are [contravariant]{.keyword #contravariant}. Of course contravariant functors are equivalent to covariant functors from the opposite category. Polymorphic functions between two contravariant functors are still natural transformations in the categorical sense, except that they work on functors from the opposite category to Haskell types.
...
We call this natural transformation the [horizontal composition]{.keyword #horizontal_composition} of $\alpha$ and $\beta$:
