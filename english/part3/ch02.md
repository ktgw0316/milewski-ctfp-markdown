...
Because of this asymmetry, the functor $L$ is called the [left_adjoint]{.keyword #left adjoint} to the functor $R$, while the functor $R$ is the right adjoint to $L$. (Of course, left and right make sense only if you draw your diagrams one particular way.)
...
To summarize what we have done: A categorical product may be defined globally as the [right_adjoint]{.keyword #right adjoint} of the diagonal functor:
