...
A monoidal category is called [symmetric]{id="symmetric"} if there is a natural isomorphism with components:
...
A monoidal category in which both are defined is called [biclosed]{id="biclosed"}. An example of a category that is not biclosed is the category of endofunctors in $\Set$, with functor composition serving as tensor product. That's the category we used to define monads.
...
An [enriched functor]{id="enriched_functor"} $F$ between two categories $\cat{C}$ and $\cat{D}$, besides mapping objects to objects, also assigns, to every pair of objects in $\cat{C}$, a morphism in $\cat{V}$:
