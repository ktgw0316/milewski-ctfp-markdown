...
Kleisli arrows form a category only if we can define proper composition for them. If there is a composition, which is associative and has an identity arrow for every object, then the functor $m$ is called a [monad]{id="monad"}, and the resulting category is called the Kleisli category.
